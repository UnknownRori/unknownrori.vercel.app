...
const useParallaxBackgroundStore = defineStore('_bg', function() {
  ...
  function rawDraw(image: ImageData, x: number, y: number, zoom: number) {
    ...
    const offsetX = (x * image.scrollSpeedX) + image.offsetX;
    const offsetY = (y * image.scrollSpeedY) + image.offsetY;
...
export default useParallaxBackgroundStore;
